...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-edit"></i> Piłkarz / Edycja #{{$player->ID_PILKARZ}}</h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('players.update', $player->ID_PILKARZ) }}" method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group @if($errors->has('IMIE')) has-error @endif">
                       <label for="IMIE-field">IMIE</label>
                    <input type="text" id="IMIE-field" name="IMIE" class="form-control" value="{{ $player->IMIE }}"/>
                       @if($errors->has("IMIE"))
                    ...
                    <input type="text" id="NAZWISKO-field" name="NAZWISKO" class="form-control" value="{{ $player->NAZWISKO }}"/>
